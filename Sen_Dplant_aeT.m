...
% Shoot-root oxygen diffusivity [m2 s-1]
D_value = [2.15*5e-9,2.15*7.5e-9,2.15*1e-8,2.15*1.5e-8,2.15*2e-8];
...
CH2O_l 	 = NaN(len*RunTime,length(D_value)*length(Aerenchyma)*length(ROLB_on_off));
...
for m = 1:length(D_value)
	
	% Shoot-root oxygen diffusivity [m2 s-1]
	Par.DO2plant    = D_value(m);
