...
% Maximum ROL barrier induction levels
Induction = 0:0.1:0.9;
...
CH2O_l 	 = NaN(len*RunTime,length(Induction)*length(Aerenchyma));


%% Scenarios of maximum ROL barrier induction levels
for m = 1:length(Induction)
    ROLB_max = Induction(m);
...
%% Plotting	
SurvTime = reshape(survive,[length(Aerenchyma) length(Induction)]);
figure(1)
imagesc(Aerenchyma,Induction,transpose(SurvTime))
xlabel('Aerenchyma content (-)');
ylabel('Max ROL barrier content (-)');
h = colorbar;
title({'Plant survival time';'Root depth 0.6 m'})
