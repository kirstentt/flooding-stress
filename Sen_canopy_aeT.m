...
% Reference canopy radius [m]
Rc_ref = 0.2; 

% Reference canopy area [m2]
Sc_ref = pi * Rc_ref^2;

% Parameters for sensitivity analysis
CanopyArea = [Sc_ref*1/2,Sc_ref*2/3,Sc_ref,Sc_ref*3/2,Sc_ref*2];
...
% Reference shoot volume [m3]
Vp_ref = pi * Par.Rp^2 * Par.Zp + pi * Rc_ref^2 * Par.Zc;

% Reference shoot dry weight [g]
ShootWeight_ref = 10; 
...
CH2O_l 	 = NaN(len*RunTime,length(CanopyArea)*length(Aerenchyma)*length(ROLB_on_off));


%% Scenarios of sensitivity analysis
for m = 1:length(CanopyArea)
	
	% Canopy area
	Par.Sc = CanopyArea(m);
	
	% Canopy radius
	Par.Rc = sqrt(Par.Sc / pi);
	...
	% Shoot dry weight [g]
	Par.shootWeight = ShootWeight_ref * (Par.Vp / Vp_ref);
