...
% Threshold of rhizosphere oxygen level for ROL barrier induction
Induction = [0.18,0.1,0.01];
...
CH2O_l 	 = NaN(len*RunTime,length(Aerenchyma)*length(RootDepth)*length(Induction));


%% Scenarios of ROL barrier induction threshold
for k = 1:length(Induction)
	
	Par.ROLB_induction = Induction(k);
...
%%Plotting
num = reshape(survive,[length(RootDepth)*length(Aerenchyma),length(Induction)]);
dim = size(num);

% Intermediate timing - Early timing
deficit_1 = num(:,2) - num(:,1);

% Early timing - Late timing
deficit_2 = num(:,1) - num(:,3);

% Intermediate timing - Late timing
deficit_3 = num(:,2) - num(:,3);
SurvTime_1 = reshape(deficit_1,[length(RootDepth),length(Aerenchyma)]);
SurvTime_2 = reshape(deficit_2,[length(RootDepth),length(Aerenchyma)]);
SurvTime_3 = reshape(deficit_3,[length(RootDepth),length(Aerenchyma)]);

figure(4)
tiledlayout(1,2);
nexttile
imagesc(RootDepth,Aerenchyma,transpose(SurvTime_1))
xlabel('Rooting depth (m)');
ylabel('Aerenchyma content');
title('Intermediate - early')
set(gca,'YDir','normal')
set(gca,'FontSize',12)
h = colorbar;
h.Layout.Tile = 'south';
h.Label.String = 'Survival time difference (h)';

nexttile
imagesc(RootDepth,Aerenchyma,transpose(SurvTime_3))
xlabel('Rooting depth (m)');
ylabel('Aerenchyma content');
title('Intermediate - late')
set(gca,'YDir','normal')
set(gca,'FontSize',12)
...
surv2=num(:,2);
surv3=num(:,3);
Surv1 = reshape(surv1,[length(RootDepth),length(Aerenchyma)]);
Surv2 = reshape(surv2,[length(RootDepth),length(Aerenchyma)]);
Surv3 = reshape(surv3,[length(RootDepth),length(Aerenchyma)]);
tiledlayout(1,3);
nexttile
imagesc(RootDepth,Aerenchyma,transpose(Surv1))
xlabel('Rooting depth (m)');
ylabel('Aerenchyma content');
title('Early')
set(gca,'YDir','normal')
...
nexttile
imagesc(RootDepth,Aerenchyma,transpose(Surv2))
xlabel('Rooting depth (m)');
ylabel('Aerenchyma content');
title('Intermediate')
set(gca,'YDir','normal')
...
nexttile
imagesc(RootDepth,Aerenchyma,transpose(Surv3))
xlabel('Rooting depth (m)');
ylabel('Aerenchyma content');
title('Late')
set(gca,'YDir','normal')
...
h = colorbar;
h.Layout.Tile = 'south';
h.Label.String = 'Survival time (h)';