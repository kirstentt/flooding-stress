...
CH2O_l 	 = NaN(len*RunTime,length(Induction)*length(RootDepth));
...
SurvTime = reshape(survive,[length(RootDepth) length(Induction)]);
figure(1)
imagesc(RootDepth,Induction,transpose(SurvTime))
xlabel('Root depth (m)');
ylabel('Max ROL barrier content');
h = colorbar;
title({'Plant survival time';'Aerenchyma content 0.5'})
