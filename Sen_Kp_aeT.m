...
% Sensitivity of stomatal closure to root ATP decline
% High K_value corresponds to high sensitivity
K_value = [2.4e-3,2.8e-3,3.2e-3,3.6e-3,4e-3];
...
% Shoot surface area
Par.Sp = (Par.Zp - Par.Zc) * (2 * pi * Par.Rp) + Par.Sc + 2 * pi ...
    * Par.Rc * Par.Zc - pi * Par.Rp^2; 
...
CH2O_l 	 = NaN(len*RunTime,length(K_value)*length(Aerenchyma)*length(ROLB_on_off));
            ...
            for i = 1:len
                ...
                for t = 1:RunTime
                    ...
                    if  CH2O > (Par.CH2O / 10)
                        ...
                        O2soil = max(O2soil + dO2soil*dt,((max(0,Par.Zr - Par.Hwater))/ Par.Zr) * Par.O2air);
						
						% Integration of bulk soil oxygen level
                        O2bulk = max(O2bulk + dO2bulk*dt,((max(0,Par.Zr - Par.Hwater))/ Par.Zr) * Par.O2air);
...
%%Plotting
figure(1)
plot(Aerenchyma,survive(1:8),'x-k','MarkerSize',12)
hold on
plot(Aerenchyma,survive(9:16),'o-k')
hold on
plot(Aerenchyma,survive(17:24),'x-r','MarkerSize',12)
hold on
plot(Aerenchyma,survive(25:32),'o-r')
hold on
plot(Aerenchyma,survive(33:40),'x-b','MarkerSize',12)
hold on
plot(Aerenchyma,survive(41:48),'o-b')
hold on
plot(Aerenchyma,survive(49:56),'x-g','MarkerSize',12)
hold on
plot(Aerenchyma,survive(57:64),'o-g')
hold on
plot(Aerenchyma,survive(65:72),'x-m','MarkerSize',12)
hold on
plot(Aerenchyma,survive(73:80),'o-m')
